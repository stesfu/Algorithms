 ...
 * Purpose    :  all logic concerning the T3 board state, including the ability to get the transitions 
 *               and available actions, and determine if the turn is for the odd / even player.
 * @author    :  Salem Tesfu 
 * Date       :  2020-02-28
 *  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
...
public class T3State {
     ...
     */
    public TreeMap<T3Action, T3State> getTransitions() {
        TreeMap<T3Action, T3State> result = new TreeMap<>();
        if (isWin() || isTie()) {
            return result;
        }
        for (int r = 0; r < 3; r++) {
            for (int c = 0; c < 3; c++) {
                for (int move : getMoves()) { // [1,3,5]
                    T3Action action = new T3Action(c, r, move);
                    if (isValidAction(action)) {
                        result.put(action, getNextState(action));
                    }
                }
            }
        }
